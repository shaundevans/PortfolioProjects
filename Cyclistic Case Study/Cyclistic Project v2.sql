...
select * from ad_days_dates add2 
...
case 
	when rideable_type = 'docked_bike' then 1
...
extract(day from started_at::timestamp) as daynum, 
extract(month from started_at::timestamp) as monthnum, 
(extract(epoch from ended_at::timestamp) - extract(epoch from started_at::timestamp))::float/3600 as duration_hours
from ad_days_dates add2 
order by duration_hours desc 


select ended_at::timestamp - started_at::timestamp as tm from ad_days_dates add2 
order by tm desc

create temp table nm_ends as 
select distinct(end_station_name), ride_id
from ad_days_dates add2 
where end_station_name not in (select distinct(start_station_name) from ad_days_dates add3)
and member_casual <> 'member'

create temp table nm_stations as 
select distinct(start_station_name), ride_id
from ad_days_dates add2 
where start_station_name not in (select distinct(end_station_name) from ad_days_dates add3 )
and member_casual <> 'member'



select * from nm_ends 
left join nm_stations using(ride_id) 


select * from nm_stations 
left join nm_ends using(ride_id)


select count(start_station_name) as count_rides, start_station_name, count(end_station_name) as ender, end_station_name, member_casual 
from ad_days_dates add2 
group by start_station_name, member_casual, end_station_name
order by count_rides desc


select avg(extract(epoch from ride_length))/3600 as day_average, member_casual, start_day 
from ad_days_dates
